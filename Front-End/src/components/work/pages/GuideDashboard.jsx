  ...
  return (
    <div className="guide-dashboard">
      <div className="container">
        ...
        {(() => {
          const activeAssign = assignments.find(a => a.status === 'upcoming' || a.status === 'ongoing');
          const isAvailable = !activeAssign;
          return (
            <div className={`availability-status-banner ${isAvailable ? 'status-available' : 'status-busy'}`}>
              <div className="status-left">
                <span className={`status-pulse ${isAvailable ? 'pulse-green' : 'pulse-red'}`}></span>
                <div className="status-text">
                  <strong>{isAvailable ? 'Available for Bookings' : 'Currently on Tour'}</strong>
                  <span>
                    {isAvailable
                      ? 'You will appear as available to customers browsing guides.'
                      : activeAssign
                        ? `On tour: ${activeAssign.tourName} — ends ${new Date(activeAssign.endDate).toLocaleDateString('en-US', { month: 'long', day: 'numeric', year: 'numeric' })}`
                        : ''}
                  </span>
                </div>
              </div>
              <div className={`status-pill ${isAvailable ? 'pill-available' : 'pill-busy'}`}>
