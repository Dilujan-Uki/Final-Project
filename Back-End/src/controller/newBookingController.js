...
export const createBooking = async (req, res) => {
  try {
    ...
    const booking = new Booking({ userId: req.userId, tourId, tourName, guideName: guideName || '', participants: parseInt(participants), duration: parseInt(duration), totalPrice: parseFloat(totalPrice), extraServices: { transport: extraServices?.transport || false, meals: extraServices?.meals || false }, bookingDate: new Date(bookingDate), specialRequests: specialRequests || '', status: 'pending', paymentStatus: 'pending' });
    const saved = await booking.save();
    res.status(201).json({ success: true, message: 'Booking created successfully', data: { id: saved._id, tourName: saved.tourName, bookingDate: saved.bookingDate, status: saved.status, totalPrice: saved.totalPrice, guideName: saved.guideName } });
  } catch (error) {
...
export const cancelBooking = async (req, res) => {
  try {
    ...
    await booking.save();
    res.status(200).json({ success: true, message: 'Booking cancelled successfully', data: { ...booking.toObject(), refundAmount, refundMessage: refundAmount > 0 ? `A 50% refund of $${refundAmount} will be processed to your original payment method within 5-7 business days.` : 'No refund applicable as payment was not completed.' } });
  } catch (error) {
  ...
  try {
    ...
    if (booking.guideName) {
      ...
      if (!guide) {
        ...
        if (userGuide) guide = await Guide.create({ name: userGuide.name, email: userGuide.email, password: userGuide.password, phone: userGuide.phone || '', userId: userGuide._id, dailyRate: 70, isActive: true });
      }
      if (guide) {
        const startDate = new Date(booking.bookingDate);
        const endDate = new Date(startDate);
        endDate.setDate(endDate.getDate() + booking.duration);
        await GuideAssignment.create({ guideId: guide._id, guideName: guide.name, bookingId: booking._id, tourId: booking.tourId, tourName: booking.tourName, customerName: booking.userId.name, customerEmail: booking.userId.email, customerPhone: booking.userId.phone || '', participants: booking.participants, duration: booking.duration, startDate: booking.bookingDate, endDate, specialRequests: booking.specialRequests || '', meetingPoint: 'To be confirmed with customer', status: 'upcoming' });
      }
    }
